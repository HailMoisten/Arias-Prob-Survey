...
const FlippableCoin = ({ isFlipped, face }) => {
    return (jsxs("div", { className: `relative w-20 h-20 transition-transform duration-1000`, style: { transformStyle: 'preserve-3d', transform: isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }, children: [jsx("div", { className: "absolute w-full h-full rounded-full bg-gray-400 border-2 border-gray-500 flex justify-center items-center shadow-lg", style: { backfaceVisibility: 'hidden' }, children: jsx("div", { className: "w-10 h-10 rounded-full bg-gray-500 opacity-50" }) }), jsx("div", { className: "absolute w-full h-full rounded-full bg-yellow-400 border-2 border-yellow-600 flex justify-center items-center shadow-lg", style: { backfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }, children: jsx("span", { className: "text-5xl font-bold text-yellow-800", children: face === 'H' ? '☆' : '' }) })] }));
};
